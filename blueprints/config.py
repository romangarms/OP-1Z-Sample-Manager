...
def run_config_task(changed_key):
    ...
    if changed_key == CONFIG_LOGGER_LEVEL:
        set_logger_level(app_config.get(CONFIG_LOGGER_LEVEL, "INFO"))
...
def get_config_setting(key, default=None):
    ...
    if key == CONFIG_WORKING_DIRECTORY:
...
def get_device_mount_path(device):
    ...
    if get_config_setting(CONFIG_DEVELOPER_MODE, False):
        ...
        key = CONFIG_OPZ_MOUNT_PATH if device == "opz" else CONFIG_OP1_MOUNT_PATH
    else:
        # Use auto-detected path in normal mode
        key = CONFIG_OPZ_DETECTED_PATH if device == "opz" else CONFIG_OP1_DETECTED_PATH
...
def reset_config_flask():
    ...
    delete_config_setting(CONFIG_OPZ_MOUNT_PATH, save=False)
    reset_config()
...
def get_general_config():
    ...
    general_json_path = os.path.join(OPZ_MOUNT_PATH, DIR_CONFIG, FILE_GENERAL_JSON)
    return jsonify(read_json_from_path(general_json_path))

@config_bp.route('/get-config/midi')
def get_midi_config():
    OPZ_MOUNT_PATH = get_device_mount_path("opz")
    midi_json_path = os.path.join(OPZ_MOUNT_PATH, DIR_CONFIG, FILE_MIDI_JSON)
...
def get_dmx_config():
    OPZ_MOUNT_PATH = get_device_mount_path("opz")
    dmx_json_path = os.path.join(OPZ_MOUNT_PATH, DIR_CONFIG, FILE_DMX_JSON)
